"""Prompt Execution Layer: English → enabled tools only."""
...
class PromptExecutionLayer:
    def __init__(self, workspace_dir: str = "workspace", state_dir: str = "state") -> None:
        ...
        if not self.cap_path.exists():
            self.cap_path.write_text("{}", encoding="utf-8")
    ...
    async def list_capabilities(self, context: RunContext) -> dict:
        """Return which agent abilities are currently ON."""
        return self._caps()

    @function_tool()
    async def set_role(self, context: RunContext, role: str) -> str:
        """Persist role from English, e.g. butler. Used on next session start too."""
        data = self._role()
    ...
    async def remember(self, context: RunContext, note: str) -> str:
        """Append a lesson to durable memory."""
    ...
    async def use_scope(self, context: RunContext, goal: str) -> str:
        """Plan which ENABLED tools should run for this English goal. Does not invent OFF tools."""
        caps = self._caps()
        on = [k for k, v in caps.items() if v]
        off = [k for k, v in caps.items() if not v]
        return (
            f"Goal: {goal}\n"
            f"Use only: {', '.join(on) or '(none)'}\n"
            f"Do not claim: {', '.join(off) or '(none)'}\n"
            f"Role: {self._role().get('role', 'unset')}"
        )

    @function_tool()
    async def run_cmd(self, context: RunContext, command: str) -> str:
        """Run one allowlisted command inside ./workspace only."""
        if not self._caps().get("shell_allowlist", False):
            return "shell_allowlist is OFF in capabilities.json"
